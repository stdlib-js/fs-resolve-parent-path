...
tape( 'the command-line interface resolves a path by walking parent directories', opts, function test( t ) {
	var cmd = [
		EXEC_PATH,
		fpath,
		'package.json'
	...
	function done( error, stdout, stderr ) {
		...
		if ( error ) {
			...
			str = stdout.toString();
			t.strictEqual( isAbsolutePath( str ), true, 'prints contents to `stdout`' );
